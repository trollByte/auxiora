...
export class CorrectionStore {
  ...
  addCorrection(
    correction: Omit<DetectionCorrection, 'id' | 'timestamp' | 'keywords'>,
  ): void {
  ...
  private recomputePatterns(): void {
    ...
    const groups = new Map<string, { keyword: string; fromDomain: ContextDomain; toDomain: ContextDomain; occurrences: number }>();
    for (const c of this.corrections) {
      const seen = new Set(c.keywords);
      for (const kw of seen) {
  ...
  getStats(): {
    ...
    const pairCounts = new Map<string, { from: ContextDomain; to: ContextDomain; count: number }>();
    const domainDetected = new Map<ContextDomain, number>();
    const domainCorrected = new Map<ContextDomain, number>();
    ...
    const correctionRate: Record<string, number> = {};
