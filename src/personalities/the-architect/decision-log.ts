...
export class DecisionLog {
  ...
  addDecision(
    decision: Omit<Decision, 'id' | 'timestamp' | 'tags'>,
  ): Decision {
  ...
  updateDecision(
    id: string,
    updates: Partial<Pick<Decision, 'status' | 'outcome' | 'followUpDate'>>,
  ): void {
    const decision = this.decisions.find(d => d.id === id);
    if (!decision) {
      throw new Error(`Decision not found: ${id}`);
    }
    if (updates.status !== undefined) decision.status = updates.status;
    if (updates.outcome !== undefined) decision.outcome = updates.outcome;
    if (updates.followUpDate !== undefined) decision.followUpDate = updates.followUpDate;
  }
  ...
  query(q: DecisionQuery): Decision[] {
    let results = this.decisions.filter(d => {
      if (q.domain !== undefined && d.domain !== q.domain) return false;
  ...
  getRecentForDomain(domain: ContextDomain, limit = 10): Decision[] {
    return this.decisions
  ...
  static deserialize(data: string): DecisionLog {
    ...
    try {
      const parsed = JSON.parse(data) as { decisions?: Decision[] };
