...
const VALID_TRAITS: ReadonlySet<string> = new Set<keyof TraitMix>([
...
export class CustomWeights {
  private overrides: Partial<Record<keyof TraitMix, number>> = {};
  ...
  static deserialize(data: string): CustomWeights {
    ...
    const parsed = JSON.parse(data) as Partial<Record<keyof TraitMix, number>>;
    for (const [trait, offset] of Object.entries(parsed) as Array<[keyof TraitMix, number]>) {
