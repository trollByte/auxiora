...
const NEGATIVE_EMOTIONS: ReadonlySet<EmotionalRegister> = new Set(['frustrated', 'stressed']);
 ...
 */
export function estimateIntensity(message: string, emotion: EmotionalRegister): number {
  if (emotion === 'neutral') return 0.2;
  ...
  if (exclamations >= 3) intensity += 0.15;
  else if (exclamations >= 1) intensity += 0.05;
  if (questions >= 3) intensity += 0.1;

  // CAPS intensity
  const capsRatio = message.replace(/[^A-Z]/g, '').length / Math.max(message.replace(/\s/g, '').length, 1);
  if (capsRatio > 0.5 && message.length > 10) intensity += 0.15;
  ...
  if (NEGATIVE_EMOTIONS.has(emotion)) {
    ...
    if (wordCount > 50) intensity += 0.1;
    else if (wordCount > 25) intensity += 0.05;
  }
...
export class EmotionalTracker {
  ...
  getEffectiveEmotion(): EffectiveEmotion {
    ...
    const emotionCounts = new Map<EmotionalRegister, number>();
    ...
    let predominant: EmotionalRegister = 'neutral';
    let maxCount = 0;
  ...
  private detectTrajectory(window: EmotionRecord[]): EmotionalTrajectory {
  ...
  private mode(values: EmotionalRegister[]): EmotionalRegister {
    const counts = new Map<EmotionalRegister, number>();
    for (const v of values) {
      counts.set(v, (counts.get(v) ?? 0) + 1);
    }
    let best: EmotionalRegister = 'neutral';
    let bestCount = 0;
