...
export class ContextRecommender {
  ...
  private correctionBased(
    ...
    const match = patterns.find(
      p => p.fromDomain === detectedContext.domain && p.toDomain === suggested,
    );
  ...
  private lowConfidence(
    ...
    if (detectionConfidence >= 0.5) return null;
    if (detectedContext.domain === 'general') return null;

    const scores = scoreAllDomains(userMessage);

    // Get the top 3 domains (excluding general) sorted by score descending
    const ranked = (Object.entries(scores) as Array<[ContextDomain, number]>)
      .filter(([d]) => d !== 'general')
      .sort(([, a], [, b]) => b - a)
      .slice(0, 3);
  ...
  private usagePattern(
    ...
    let topDomain: ContextDomain | null = null;
    let topCount = 0;

    for (const [domain, count] of Object.entries(usageHistory) as Array<[ContextDomain, number]>) {
    ...
    if (topDomain === null) return null;

    const percentage = topCount / total;
    if (percentage <= 0.6) return null;
